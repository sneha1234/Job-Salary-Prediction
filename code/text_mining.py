...
df = pd.read_csv('C:/sneha/spring16/Data_Science/project/code/temp1.csv')#read data to a dataframe
df1 = pd.DataFrame(data=df,columns=['ContractType', 'ContractTime','Company','Category','SourceName'])#take only slected columns
dd = pd.get_dummies(df1, prefix=['ContractType', 'ContractTime','Company','Category','SourceName'])#dummy code those columns
df=df.drop('ContractType',axis=1) #drop the original columns from the main dataframe 
df=df.drop('ContractTime',axis=1)#..................................................
...
for i in range(0,len(df.index)):
	L=geolocator.geocode(df['LocationRaw'][i],timeout=None)
	if L is not None:
...
result=pd.concat([result,newdf],axis=1)
result=result.drop('LocationRaw',axis=1)
##title processing
...
result.to_csv("C:/sneha/spring16/Data_Science/project/code/x.csv",index=False)
